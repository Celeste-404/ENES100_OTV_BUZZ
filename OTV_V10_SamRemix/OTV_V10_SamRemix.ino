...
void obstacleLoop(){
  ...
  if(Enes100.getTheta()<0.15 && Enes100.getTheta()>-0.15){
    ...
    if (measureDistance(7,A4)<30){
      stopMotors();
      if(Enes100.getY() < 1){ //More distance to the left 
    ...
    Enes100.println("I'm facing north"); // CREATE FORWARD UNTILK CLEA
    while(measureDistance(8,A3) <= 40)
    {
      startMotorsFWD(500,150);
      stopMotors();
      delay(700);
      Enes100.println("im still not in the clear");
    }
    Enes100.println("I found a spot TURNING TOWARDS IT");
    startMotorsFWD(2000, 170);
    turn(0);
    if (measureDistance(7,A4)<30){
      stopMotors();
